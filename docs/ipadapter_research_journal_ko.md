...
## 2026-06-13 c085 anchored full-adapter 학습 및 ComfyUI gate

c084는 crop-pair 신호가 QwenVL metric상 일부 살아 있었지만, calibrator-only 범위가 좁아 frog/yokai/chibi reference를 성인형 green humanoid template로 바꾸는 문제가 남았다. c085의 목적은 같은 crop-pair 데이터를 anchor 데이터와 섞고, full adapter 범위를 trainable로 열면 기존 최고 기준 `blend_species_face`를 넘을 수 있는지 확인하는 것이었다.

### 사용 데이터와 manifest

- crop 원천: `training/manifests/c084_balanced_crop_pairs_20260613.jsonl`
- anchor 원천: `training/manifests/c060_qwenvl_failure_focused_clean32_c052_20260612.jsonl`
- 새 manifest: `training/manifests/c085_anchored_full_adapter_20260613.jsonl`
- summary: `training/manifests/c085_anchored_full_adapter_20260613.summary.json`
- materialized image root: `.tmp/c085_anchored_full_adapter_root`

`tools/c085_anchored_full_adapter_manifest.py`를 만들어 c084 crop rows 80개, clean anchor 32개, c052 positive anchor 16개, failure anchor 32개를 합친 160 row manifest를 구성했다. heldout row는 0개였고, 각 target image의 caption txt도 같이 materialize해서 기존 trainer가 바로 읽을 수 있게 했다.
...
- finite loss: true
- report: `eval/qwenvl_c085_anchored_full_adapter_training_20260613/report.md`

첫 시도에서는 target caption txt가 scratch root에 없어 trainer가 실패했다. manifest builder를 수정해 source root의 `.txt` caption을 symlink하거나 prompt fallback을 쓰게 만들고, 테스트로 jpg/txt materialization을 고정했다.
...
- contact sheets:
  - `eval/qwenvl_c085_anchored_full_adapter_gate_20260613/contact_sheet_train.jpg`
  - `eval/qwenvl_c085_anchored_full_adapter_gate_20260613/contact_sheet_heldout.jpg`
  - `eval/qwenvl_c085_anchored_full_adapter_gate_20260613/contact_sheet_crop_pair_focus.jpg`

ComfyUI object_info에서 c085 checkpoint가 `AnimaQwenVLIPAdapterLoader` 선택지로 정상 노출되는 것을 확인했다. 생성 후 server를 종료했고 8116 port가 닫힌 것도 확인했다.

### 결과 판단

clean32+heldout8에서는 c085가 c084 대비 PE 평균 uplift는 조금 높였지만, 기존 최고 `blend_species_face`에는 미치지 못했다. QwenVL 기준에서는 c084보다도 낮았다.
...
heldout8만 보면 c085는 PE에서 c084보다 낫지만, QwenVL에서는 c084보다 낮고 blend보다도 낮다.

| encoder | variant | mean uplift | improved rate |
|---|---|---:|---:|
| PE | `blend_species_face` | `0.0535339788` | `0.875` |
| PE | `c084_balanced_crop_pair_w14` | `-0.0063817352` | `0.625` |
| PE | `c085_anchored_full_adapter_w14` | `0.0370979905` | `0.750` |
| QwenVL | `blend_species_face` | `0.0264708772` | `0.750` |
| QwenVL | `c084_balanced_crop_pair_w14` | `0.0235493183` | `0.625` |
| QwenVL | `c085_anchored_full_adapter_w14` | `0.0192572623` | `0.750` |

crop-pair focus에서는 QwenVL 평균 uplift가 c085 `0.0652211845`, blend `0.0649063945`로 거의 같은 수준에서 c085가 아주 근소하게 높았다. 하지만 PE 기준은 c085가 `0.1288137928`로 c084 `0.1503805324`, blend `0.1395019785`보다 낮았다. 사람이 봐도 c085는 초록 피부와 강한 얼굴 대비는 유지하지만, frog/yokai/chibi의 작은 체형, headwear, mascot silhouette, side-profile 정보를 adult green humanoid template로 바꾼다.

따라서 c085는 `runtime pass / quality fail`이며 high-quality reference-control checkpoint로 승격하지 않는다. 다음 루프는 full adapter continuation 반복보다, hard-negative 생성 결과와 reference crop을 직접 대조하는 objective 또는 encoder-side feature adaptation으로 이동해야 한다.

### c085 관련 파일

- `tools/c085_anchored_full_adapter_manifest.py`
- `tests/test_c085_anchored_full_adapter_manifest.py`
- `training/manifests/c085_anchored_full_adapter_20260613.jsonl`
- `training/manifests/c085_anchored_full_adapter_20260613.summary.json`
- `eval/qwenvl_c085_anchored_full_adapter_training_20260613/report.md`
- `eval/qwenvl_c085_anchored_full_adapter_training_20260613/summary.json`
- `eval/qwenvl_c085_anchored_full_adapter_gate_20260613/report.md`
- `eval/qwenvl_c085_anchored_full_adapter_gate_20260613/visual_audit.md`
- `eval/qwenvl_c085_anchored_full_adapter_gate_20260613/pe_similarity_metrics.json`
...
## 21. c088 Shape/Silhouette Feature Separability Probe

c087은 expanded crop-positive supervision을 늘리고 full adapter까지 풀어도 hard-shape reference gate를 넘지 못했다. 특히 frog mascot, yokai/chibi, non-human profile 같은 reference가 초록 피부나 비인간 분위기 정도만 반영되고, 둥근 몸체, 짧은 비율, 개구리형 얼굴, 실루엣은 adult humanoid template로 무너졌다. 그래서 c088의 목적은 같은 adapter continuation을 반복하기 전에, 현재 encoder embedding이 형태/실루엣 신호를 실제로 분리할 수 있는지 확인하는 것이었다.

### 검증 목표

- c087 hard-shape 실패 사례에서 reference와 generated variant 사이의 형태/실루엣 신호가 metric으로 잡히는지 본다.
- QwenVL, SigLIP2, PE embedding 중 어떤 공간이 hard-shape variant 선택에 유효한지 비교한다.
- heldout 학습 누수 없이 다음 실험이 broad adapter continuation인지, explicit shape distillation인지, encoder-side checkpoint adaptation인지 결정한다.
...
- source: c087 gate 산출물 중 crop-focus 10 row + heldout07 1 row
- 총 row: `11`
- crop-focus row: `10`
- heldout eval row: `1`
- heldout training row used: `0`
- shape group:
  - `frog_yokai_guard`: `10`
  - `heldout07_non_human_profile`: `1`
- 비교 variant:
  - `no_ip`
  - `blend_species_face`
...
heldout07은 평가 전용 hard case로만 사용했고, c088는 새 학습이 아니라 probe/evaluation이므로 heldout 이미지를 training manifest에 넣지 않았다.

### 개발한 도구

- `tools/c088_probe_io.py`: manifest/JSON/path 입출력 공통 처리
- `tools/c088_probe_manifest.py`: c087 산출물에서 c088 probe manifest와 summary 생성
- `tools/c088_shape_metrics.py`: edge/projection/silhouette 기반 shape metric 계산과 contact sheet 생성
- `tools/c088_embedding_metrics.py`: QwenVL, SigLIP2, PE embedding similarity 평가
- `tools/c088_report.py`: feature별 support rate를 모아 rollup/report 생성
- `tools/c088_shape_silhouette_probe.py`: c088 CLI entrypoint
- `tests/test_c088_shape_silhouette_probe.py`: manifest 생성, shape metric, embedding scorer, report rollup 회귀 테스트
- plan doc: `docs/c088_shape_silhouette_feature_probe_plan_ko.md`

### 실행한 명령

```sh
PYTHONPATH=. /home/wktwin/anima-lora-training-bundle/anima_lora/.venv/bin/python tools/c088_shape_silhouette_probe.py build --crop-limit 10
PYTHONPATH=. /home/wktwin/anima-lora-training-bundle/anima_lora/.venv/bin/python tools/c088_shape_silhouette_probe.py score-shape
PYTHONPATH=. /home/wktwin/anima-lora-training-bundle/anima_lora/.venv/bin/python tools/c088_embedding_metrics.py eval/c088_shape_silhouette_feature_probe_20260613/probe_manifest.jsonl eval/c088_shape_silhouette_feature_probe_20260613/qwenvl_embedding_metrics.json --encoder qwenvl
PYTHONPATH=. /home/wktwin/anima-lora-training-bundle/anima_lora/.venv/bin/python tools/c088_embedding_metrics.py eval/c088_shape_silhouette_feature_probe_20260613/probe_manifest.jsonl eval/c088_shape_silhouette_feature_probe_20260613/siglip_embedding_metrics.json --encoder siglip --device cuda
PYTHONPATH=.:/home/wktwin/anima-lora-training-bundle/anima_lora /home/wktwin/anima-lora-training-bundle/anima_lora/.venv/bin/python tools/c088_embedding_metrics.py eval/c088_shape_silhouette_feature_probe_20260613/probe_manifest.jsonl eval/c088_shape_silhouette_feature_probe_20260613/pe_embedding_metrics.json --encoder pe --device cuda
PYTHONPATH=. /home/wktwin/anima-lora-training-bundle/anima_lora/.venv/bin/python tools/c088_report.py --output-dir eval/c088_shape_silhouette_feature_probe_20260613
```

### 결과

rollup decision은 `shape_signal_present_encoder_embedding_not_enough`이다. edge/projection/silhouette metric과 PE에는 일부 형태 신호가 있지만, native adapter 경로에서 기대한 QwenVL/SigLIP2 embedding은 c087 hard-shape set에서 support threshold를 넘지 못했다.

| feature | cases | supported | support rate | source decision |
|---|---:|---:|---:|---|
| `edge_projection_silhouette` | `11` | `6` | `0.5454545455` | `shape_silhouette_signal_viable` |
| `qwenvl` | `11` | `4` | `0.3636363636` | `embedding_signal_not_viable` |
| `siglip2` | `11` | `4` | `0.3636363636` | `embedding_signal_not_viable` |
| `pe` | `11` | `7` | `0.6363636364` | `embedding_signal_viable` |

heldout07 hard case에서는 QwenVL이 `no_ip`를 best로 두었고, SigLIP2는 c086을 best로 고르긴 했지만 uplift가 `0.0249527097`에 그쳐 `embedding_signal_not_enough`로 판정되었다. PE만 c086을 best로 두고 uplift `0.0716669559`, margin `0.0636019111`로 supervised objective에 쓸 수 있는 신호를 보였다.

| feature | best variant | uplift | margin | decision |
|---|---|---:|---:|---|
| `edge_projection_silhouette` | `c087_expanded_crop_positive_w14` | `0.0246184729` | `0.0145832580` | `shape_signal_not_enough` |
| `qwenvl` | `no_ip` | `0.0` | `0.0096047521` | `embedding_signal_not_enough` |
| `siglip2` | `c086_hard_negative_w14` | `0.0249527097` | `0.0249527097` | `embedding_signal_not_enough` |
| `pe` | `c086_hard_negative_w14` | `0.0716669559` | `0.0636019111` | `embedding_signal_supports_supervised_objective` |

### 판단

c088은 “IP-Adapter 자체가 무조건 불가능하다”는 결론은 아니다. 다만 현재 QwenVL/SigLIP2 embedding을 그대로 adapter head에 넣고 continuation하는 방식은 hard-shape reference-control을 안정적으로 해결하기 어렵다는 증거가 추가되었다. 특히 c087처럼 full adapter를 크게 학습해도 형태가 adult humanoid template로 수렴하는 문제는, adapter head보다 encoder feature/objective 쪽 병목일 가능성이 크다.

따라서 다음 루프는 다음 중 하나여야 한다.

1. edge/projection/silhouette 또는 PE 신호를 QwenVL/SigLIP 경로에 distill하는 explicit shape objective
2. SigLIP2/QwenVL embedding 앞단 또는 projection 옆에 붙는 encoder-side shape checkpoint/feature adapter
3. hard-shape subset을 별도 학습 대상으로 둔 작은 pilot 후, ComfyUI contact-sheet gate로 실제 생성 품질 확인

같은 expanded crop-positive full-adapter continuation을 반복하는 것은 우선순위에서 제외한다.

### c088 관련 파일

- `docs/c088_shape_silhouette_feature_probe_plan_ko.md`
- `eval/c088_shape_silhouette_feature_probe_20260613/probe_manifest.jsonl`
- `eval/c088_shape_silhouette_feature_probe_20260613/summary.json`
- `eval/c088_shape_silhouette_feature_probe_20260613/shape_silhouette_metrics.json`
- `eval/c088_shape_silhouette_feature_probe_20260613/qwenvl_embedding_metrics.json`
- `eval/c088_shape_silhouette_feature_probe_20260613/siglip_embedding_metrics.json`
- `eval/c088_shape_silhouette_feature_probe_20260613/pe_embedding_metrics.json`
- `eval/c088_shape_silhouette_feature_probe_20260613/metrics.json`
...
## 2026-06-13 C094 SigLIP shape-supervised anti-collapse loop

### 왜 시작했나

C093는 explicit negative를 실제 loss에 연결해서 C092보다 아주 작은 수치 개선을 만들었지만, contact sheet에서는 green human face collapse가 계속 남았다. 특히 frog/chibi/mascot row가 전부 비슷한 녹색 인간 얼굴로 빨려 들어갔고, heldout07의 monster side-profile도 보존되지 않았다.

그래서 C094의 목표는 단순 contrastive continuation이 아니었다. C093 checkpoint를 시작점으로 두고, target/reference latent의 edge/projection shape loss를 추가해서 "참조 이미지의 큰 실루엣과 구조가 생성 target 쪽으로 더 강하게 전달되는지"를 확인했다. heldout07은 학습에서 제외했고, c092/c093/Qwen baseline과 같은 ComfyUI API gate에서 비교했다.
...
- explicit negative rows: `10`
- heldout07: 학습에서 제외
- init checkpoint: `checkpoints/anima_siglip_ip_adapter_c093_qwen_target_anti_collapse_0048_20260613.safetensors`
- output checkpoint: `checkpoints/anima_siglip_ip_adapter_c094_shape_supervised_0064_20260613.safetensors`

C094 checkpoint는 promotion되지 않은 830MB 실험 artifact이므로 `.gitignore`에서 `checkpoints/*shape_supervised*.safetensors`로 local-only 처리했다.

### 개발한 것

- `training/latent_shape_loss.py`
  - denoised latent에서 edge/projection 기반 shape loss를 계산한다.
  - scheduler `sigma` shape가 0D/1D/4D로 들어와도 broadcast되도록 처리했다.
- `training/siglip_shape_cache.py`
  - reference image latent를 cache해서 shape loss 계산에 재사용한다.
- `training/siglip_shape_contrastive_smoke.py`
  - C093 contrastive 학습 루프에 target-side latent shape supervision과 reference shape loss를 추가했다.
- `training/siglip_shape_contrastive_cli.py`
  - C094 실행용 CLI를 추가했다.
- `tools/c094_shape_training_report.py`
  - 학습 stdout JSON을 파싱해서 training gate report/summary를 생성한다.
- `tools/c094_shape_eval.py`
  - ComfyUI API로 no_ip, 기존 SigLIP, C089, C091, C092, C093, C094 sweep을 한 contact sheet에서 비교한다.
- `tools/c094_shape_metrics.py`
  - C094/C093/C092/Qwen baseline uplift, heldout07, diversity proxy, blank-like row를 집계한다.
- tests:
  - `tests/test_latent_shape_loss.py`
  - `tests/test_siglip_shape_cache.py`
  - `tests/test_siglip_shape_contrastive_smoke.py`
  - `tests/test_c094_shape_training_report.py`
  - `tests/test_c094_shape_eval.py`
...
PYTHONPATH=. /home/wktwin/anima-lora-training-bundle/anima_lora/.venv/bin/python training/siglip_shape_contrastive_cli.py \
  --manifest-path training/manifests/c093_siglip_qwen_target_anti_collapse_20260613.jsonl \
  --image-root .tmp/c093_anti_collapse_root \
  --init-checkpoint-path checkpoints/anima_siglip_ip_adapter_c093_qwen_target_anti_collapse_0048_20260613.safetensors \
  --output-path checkpoints/anima_siglip_ip_adapter_c094_shape_supervised_0064_20260613.safetensors \
  --steps 64 \
  --max-rows 10 \
  --resolution 256 \
  --device cuda:0 \
...
- decision: `proceed_to_c094_generation_gate`
- steps: `64`
- rows loaded: `10`
- explicit negative rows: `10`
- first loss: `0.2693069875`
- final loss: `0.0954880044`
- mean loss: `0.1448897758`
- mean base loss: `0.1256594866`
- mean contrastive loss: `0.0766582569`
- mean shape loss: `0.0003286188`
- finite loss: `true`
- trainable parameters: `217,369,756`
- frozen base parameters: `2,913,827,059`
- checkpoint loadable: `true`
...
isolated ComfyUI 서버는 port `8120`에서 실행했다. repo custom node symlink와 `tools/comfyui_extra_model_paths.yaml`를 사용해서 C094 checkpoint가 `AnimaSigLIPIPAdapterLoader` selector에 표시되는 것을 확인했다.

generation gate:

```sh
PYTHONPATH=. /home/wktwin/anima-lora-training-bundle/anima_lora/.venv/bin/python tools/c094_shape_eval.py \
  eval/c088_shape_silhouette_feature_probe_20260613/probe_manifest.jsonl \
  --out-dir eval/c094_siglip_shape_supervised_anti_collapse_generation_gate_20260613 \
  --data-root .tmp/c094_siglip_hard_shape_root \
  --comfy-input .tmp/comfy_siglip_c094/input \
  --comfy-output .tmp/comfy_siglip_c094/output \
...
- samples: `11`
- variants: `11`
- contact sheet: `eval/c094_siglip_shape_supervised_anti_collapse_generation_gate_20260613/contact_sheet_hard_shape.jpg`
- metric rollup: `eval/c094_siglip_shape_supervised_anti_collapse_generation_gate_20260613/metric_rollup.json`
- pixel audit: `eval/c094_siglip_shape_supervised_anti_collapse_generation_gate_20260613/pixel_nonblank_audit.json`
- visual audit: `eval/c094_siglip_shape_supervised_anti_collapse_generation_gate_20260613/visual_audit.md`
- cleanup receipt: `eval/c094_siglip_shape_supervised_anti_collapse_generation_gate_20260613/cleanup_port_8120.txt`

pixel audit는 `generated_count=121`, `blank_count=1`, `low_variance_count=1`을 기록했다. 저분산 이미지는 `crop_pair00_no_ip` baseline이고 C094 이미지는 blank-like row가 없다.
...
| `c092_qwen_target_w14` | `0.0852681653` | `1.0` | `11` |
| `c093_anti_collapse_w14` | `0.0863735780` | `1.0` | `11` |
| `c094_shape_supervised_w08` | `0.0575968148` | `0.7272727273` | `11` |
| `c094_shape_supervised_w10` | `0.0788191799` | `0.8181818182` | `11` |
| `c094_shape_supervised_w12` | `0.0823279162` | `0.8181818182` | `11` |
| `c094_shape_supervised_w14` | `0.0878832954` | `0.9090909091` | `11` |
| `c087_expanded_crop_positive_w14` | `0.1089544056` | `0.9090909091` | `11` |

### 결과 판단

decision은 `c094_shape_supervision_exhausted_requires_encoder_training`이다.

C094 w14는 mean uplift `0.0878832954`로 C093 w14 `0.0863735780`보다 `+0.0015097174` 높다. 하지만 C094 gate 기준인 `>=0.095`, C093 대비 `+0.008`을 모두 넘지 못했다. Qwen target baseline `0.1089544056`과의 격차도 여전히 크다.

heldout07은 C094 best uplift `0.0085623513`으로 C093 `0.0045189584`, C092 `0.0009986630`보다는 올라갔다. 하지만 기준 `0.025`와 C093 대비 `+0.020`에는 크게 부족하다.

visual audit에서도 같은 결론이다. C094는 이미지를 생성하고 색/질감은 안정적이지만, frog/chibi/mascot/non-human row를 대부분 녹색 인간 얼굴/흉상으로 바꾼다. 작은 몸, 큰 눈, 개구리 머리, 단순화된 cartoon silhouette 같은 reference identity가 유지되지 않는다. heldout07도 red eye와 측면 구도 느낌은 조금 남지만 monster jaw/silhouette가 유지되지 않고 human warrior portrait로 이동한다.

따라서 C094는 "shape supervision을 더하면 조금 나아지는가"라는 질문에는 yes에 가깝지만, "adapter-only 튜닝으로 고퀄 reference-control을 달성할 수 있는가"에는 no에 가깝다. 다음 루프는 C095로 분리하고, 단순 loss weight 조정보다는 image encoder / feature bridge adaptation을 직접 학습해야 한다.

### 다음 결정: C095

C095의 방향은 adapter-only continuation이 아니다. 목표는 SigLIP image feature가 non-human shape/species identity를 잃기 전에 보존하도록, encoder-side feature adapter 또는 projection bridge를 학습하는 것이다.

우선순위:

1. C094 결과를 기준선으로 고정한다.
2. SigLIP image embedding과 target/Qwen-positive embedding 사이에 작은 feature bridge 또는 residual adapter를 둔다.
3. hard negative는 C093/C094에서 확인한 collapse output을 계속 사용한다.
4. gate는 C094와 같은 hard-shape ComfyUI API sheet를 사용하되 promotion 기준을 유지한다.
5. C095도 끝나면 training/generation/visual audit/research journal/commit/push를 한 세트로 처리한다.
...
- `tests/test_c094_shape_training_report.py`
- `tests/test_c094_shape_eval.py`
- `eval/c094_siglip_shape_supervised_anti_collapse_training_20260613/train_stdout.txt`
- `eval/c094_siglip_shape_supervised_anti_collapse_training_20260613/training_summary.json`
- `eval/c094_siglip_shape_supervised_anti_collapse_training_20260613/summary.json`
- `eval/c094_siglip_shape_supervised_anti_collapse_training_20260613/report.md`
- `eval/c094_siglip_shape_supervised_anti_collapse_training_20260613/object_info.json`
- `eval/c094_siglip_shape_supervised_anti_collapse_generation_gate_20260613/summary.json`
- `eval/c094_siglip_shape_supervised_anti_collapse_generation_gate_20260613/contact_sheet_hard_shape.jpg`
- `eval/c094_siglip_shape_supervised_anti_collapse_generation_gate_20260613/metric_rollup.json`
- `eval/c094_siglip_shape_supervised_anti_collapse_generation_gate_20260613/pixel_nonblank_audit.json`
...
## 2026-06-13 C102 stronger VLM QA teacher gate

### 왜 시작했나

C101은 C100 local-real 후보 `64`개를 모두 라벨링했지만 `local_positive=0`이었다. 이전 단계의
QwenVL scorer는 embedding similarity 기반이었고, c070은 sidecar caption/heuristic search였기 때문에
실제 이미지에 질문을 던지는 생성형 VLM QA teacher가 아니었다. 따라서 C102의 목적은 학습이 아니라,
로컬에 있는 생성형 Qwen3-VL 모델로 64개 후보를 직접 질의해 direct-green/non-human 양성이 있는지
재확인하는 것이었다.
...
- checkpoint 생성 없음
- external/synthetic 후보를 local-real greenlight 증거로 사용하지 않음
- 기존 C101 `local_negative`를 VLM 단독 응답만으로 학습 positive로 뒤집지 않음

C103 greenlight 조건은 다음과 같이 두었다.

- `candidate_rows == covered_rows == 64`
- `confirmed_local_positive_count >= 8`
- `teacher_only_positive_count == 0`
- `heldout_leakage_count == 0`
- `missing_path_count == 0`
...
repo 내부 조사와 로컬 모델 확인 결과는 다음과 같다.

- `tools/build_reference_prompt_manifest.py`, `tools/score_auto_caption_qwenvl_metrics.py`
  - Qwen3-VL Embedding 기반 image-text scoring이다.
  - caption/QA 생성 모델이 아니라서 C102의 stronger QA teacher로는 부족하다.
- `tools/c070_qwenvl_caption_search.py`
  - sidecar caption과 color heuristic 기반 검색이다.
  - 이미지 자체에 질문하지 않으므로 C102 QA teacher가 아니다.
- `/data/ai/models/LLM/Qwen-VL/Qwen3-VL-2B-Thinking`
  - `transformers`에서 로드와 이미지 입력 generation이 가능했다.
  - 하지만 Thinking 모델이라 `LABEL:` 형식을 잘 지키지 않고 긴 reasoning으로 흐르는 경향이 있었다.
- `/data/ai/models/LLM/Qwen-VL/Qwen3-VL-8B-Instruct`
  - HF-format local generative VLM로 로드 가능했다.
  - 2B-Thinking보다 compact label 응답이 안정적이라 C102 teacher로 선택했다.

### 개발한 것

- `tools/c102_vlm_teacher_core.py`
  - Qwen 응답에서 `direct_green_non_human`, `human_character`, `green_background_or_object`,
    `proxy_only`, `unclear` 라벨을 보수적으로 파싱한다.
  - C101 `local_negative`는 보존하고, VLM이 direct positive라고 해도 conflict-free positive로 세지 않는다.
  - summary와 greenlight/block decision을 계산한다.
- `tools/c102_qwen3vl_teacher.py`
  - 로컬 Qwen3-VL-8B-Instruct를 `transformers` + `qwen_vl_utils`로 로드해 이미지 QA를 실행한다.
- `tools/c102_vlm_teacher_gate.py`
  - C101 후보 64개를 읽고 QA prompt/response/reviewed manifest/summary/report/source inventory를 생성한다.
  - `--response-source`를 지원해 실제 VLM 없이도 replay/test가 가능하게 했다.
- `tools/c102_review_sheet.py`
  - QA label과 final label을 함께 보는 review sheet를 만든다.
- `tools/c102_vlm_teacher_text.py`
  - C102 plan/report markdown을 생성한다.
- `tests/test_c102_vlm_teacher_gate.py`
  - 8개 conflict-free QA positive가 있을 때만 `c103_training_greenlit`이 되는지 검증한다.
  - 기존 C101 negative를 VLM 단독 direct-positive 응답으로 뒤집지 않는지 검증한다.
  - Qwen 응답 형식 변형을 parser가 보수적으로 처리하는지 검증한다.

실행 명령:

```bash
PYTHONPATH=. TRANSFORMERS_VERBOSITY=error \
  /home/wktwin/anima-lora-training-bundle/anima_lora/.venv/bin/python \
  tools/c102_vlm_teacher_gate.py \
  --out-dir eval/c102_stronger_vlm_qa_teacher_gate_20260613 \
  --model-path /data/ai/models/LLM/Qwen-VL/Qwen3-VL-8B-Instruct
```
...
review sheet는
`eval/c102_stronger_vlm_qa_teacher_gate_20260613/c102_teacher_review_sheet.jpg`에 만들었다.
크기는 `1040x5088`이다.

시각 확인 결과도 summary와 일치했다. 후보들은 컵, 잎/배경, 말풍선 주변 녹색, 일반 인간 얼굴/상반신,
수염/측면/창백한 피부 proxy가 대부분이었다. Qwen3-VL이 `direct_green_non_human`을 하나도 제안하지
않은 것은 현재 후보 풀에서는 합리적인 판정으로 보인다.

decision은 `c103_blocked_needs_manual_annotation_or_external_teacher`이다.

### 판단

C102 결과는 이제 더 명확하다. 문제는 “Qwen/SigLIP IPAdapter 학습을 조금 더 돌리면 바로 해결된다”가
아니라, 현재 local color candidate pool 안에 C103 학습을 시작할 direct-green/non-human positive가
없다는 점이다. C100 pixel mining, C101 prior visual review, C102 Qwen3-VL QA가 모두 같은 방향을 가리킨다.

따라서 이 candidate pool로 C103 학습을 시작하면 green object/human/proxy를 positive로 잘못 먹이는
위험이 크다. 다음 루프는 다음 중 하나여야 한다.

1. 사람이 C102 review sheet를 직접 보고 `local_positive`를 최소 8개 이상 확정한다.
2. local-real dataset 바깥의 external/synthetic direct-green/non-human positive track을 명확히 분리해서
   학습 후보를 구성한다.
3. local-real 데이터셋 자체에서 새로운 query/segmentation 방식으로 후보를 다시 mining하되, green object
   false positive를 먼저 줄이는 negative filter를 추가한다.

### c102 관련 파일

- `docs/c102_stronger_vlm_qa_teacher_gate_plan_ko.md`
- `tools/c102_vlm_teacher_core.py`
- `tools/c102_qwen3vl_teacher.py`
- `tools/c102_vlm_teacher_gate.py`
- `tools/c102_review_sheet.py`
- `tools/c102_vlm_teacher_text.py`
- `tests/test_c102_vlm_teacher_gate.py`
- `eval/c102_stronger_vlm_qa_teacher_gate_20260613/source_inventory.json`
- `eval/c102_stronger_vlm_qa_teacher_gate_20260613/c102_vlm_qa_prompts.jsonl`
- `eval/c102_stronger_vlm_qa_teacher_gate_20260613/c102_vlm_qa_responses.jsonl`
- `eval/c102_stronger_vlm_qa_teacher_gate_20260613/c102_teacher_reviewed_manifest.jsonl`
- `eval/c102_stronger_vlm_qa_teacher_gate_20260613/c102_teacher_summary.json`
- `eval/c102_stronger_vlm_qa_teacher_gate_20260613/c102_teacher_review_sheet.jpg`
- `eval/c102_stronger_vlm_qa_teacher_gate_20260613/c102_decision_report.md`

## 2026-06-13 C103 post-C102/C098 next branch gate

### 왜 시작했나

C102는 local-real 후보 64개를 Qwen3-VL-8B-Instruct로 다시 QA했지만
`confirmed_local_positive_count=0`이었다. 최소 greenlight 기준은 `8`개였기 때문에, 현재 C100-C102
candidate pool로 학습을 시작하면 녹색 배경/소품/일반 인간/proxy를 positive로 먹이는 위험이 크다.

동시에 C098은 C097의 56-row hard-shape explicit-negative 데이터를 사용해 deeper SigLIP encoder-LoRA를
학습했지만 승격하지 못했다. 기능적으로는 load/generate가 되었지만, `c098_lora_c094_w14` mean uplift는
`0.0865313863`으로 C096 `0.0880849553`, C094 `0.0878832954`, Qwen baseline `0.1089544056`보다 낮다.

따라서 C103은 또 다른 blind training이 아니라 다음 실험 분기를 결정하는 gate로 열었다.

### inventory 결과

생성한 inventory:

- `eval/c103_post_c102_c098_branch_gate_20260613/source_inventory.json`

핵심 값:

| 항목 | 값 |
|---|---:|
| required artifact missing path count | `0` |
| C102 candidate rows | `64` |
| C102 confirmed local positive | `0` |
| C102 min confirmed positive | `8` |
| C074 external real target positives | `10` |
| C079 total target positives | `33` |
| C080 paired training rows | `196` |
| C087 expanded crop rows | `224` |
| C097 selected hard-shape rows | `56` |
| C097 explicit negatives | `56` |
| C098 rows loaded | `56` |
| C098 explicit negatives | `56` |
| C098 best mean uplift | `0.0865313863` |
| Qwen hard-shape baseline mean uplift | `0.1089544056` |

### 결정

생성한 결정 파일:

- `eval/c103_post_c102_c098_branch_gate_20260613/branch_decision.json`
- `docs/c103_post_c102_c098_next_branch_decision_ko.md`

선택 branch는 `c104_expanded_qwen_target_siglip_distillation_gate`다.

이유는 C092의 Qwen-target distillation이 SigLIP 계열에서 가장 큰 개선 신호를 냈기 때문이다.
C092 w14는 mean uplift `0.0852681653`까지 올라갔고, 이후 C093/C094/C095/C096/C098이 여러 보정을
시도했지만 Qwen baseline에는 계속 미치지 못했다. C104는 C092를 단순 반복하지 않고, C097의 더 큰
56-row hard-shape/explicit-negative 데이터에 Qwen target supervision을 확장하는 manifest/probe부터
진행한다.

### 버린 분기

1. C100-C102 local-real 후보 학습
   - local positive가 `0`개라 학습 positive로 안전하지 않다.
2. C098과 같은 SigLIP encoder-LoRA 반복
   - C098은 학습 loss와 runtime은 성공했지만 생성 품질은 승격하지 못했다.
3. C074 external positive 단독 학습
   - target positive `10`개는 유용하지만 license/NFA caveat와 좁은 캐릭터 편향이 있어 단독 메인 branch로 쓰지 않는다.

### 다음 루프

C104는 먼저 `expanded Qwen-target SigLIP hard-shape distillation probe`로 진행한다.
...
- C104 probe가 C098 완료 LoRA보다 강한 feature margin을 보일 것

probe가 통과하면 bounded training과 C098-style generation gate로 간다. 실패하면 학습하지 않고
blocker를 남긴다. 어느 쪽이든 실험 완료 후 문서화, 검증, 커밋, 푸시를 진행한다.

## 2026-06-13 C104 expanded Qwen-target SigLIP hard-shape probe

### 왜 시작했나

C103에서 다음 분기를 `c104_expanded_qwen_target_siglip_distillation_gate`로 정했다. 다만 C098이 이미
C097 56-row hard-shape explicit-negative 데이터로 deeper SigLIP encoder-LoRA를 학습했는데도 승격하지
못했기 때문에, C104는 곧바로 긴 학습을 시작하지 않았다. 먼저 C097 데이터 전체에서 SigLIP2 feature
공간이 positive target과 explicit negative를 충분히 분리하는지 확인하는 probe로 시작했다.

### 사용 데이터와 기준

- source manifest: `training/manifests/c097_siglip_hard_shape_expanded_pairs_20260613.jsonl`
- image root: `.tmp/c097_siglip_hard_shape_expanded_root`
- C104 output: `eval/c104_expanded_qwen_target_siglip_probe_20260613/`
- selected rows: `56`
- positive token rows: `56`
- explicit negative token rows: `56`
- heldout rows used: `0`
- missing path count: `0`
- C098 best mean uplift 기준: `0.0865313863`
- C087 Qwen hard-shape baseline mean uplift 기준: `0.1089544056`

### 개발한 것

C104 전용 도구와 테스트를 추가했다.

- `tools/c104_expanded_qwen_target_siglip_probe.py`
- `tests/test_c104_expanded_qwen_target_siglip_probe.py`
- `docs/c104_expanded_qwen_target_siglip_probe_plan_ko.md`

도구의 역할은 두 가지다.

1. C097의 `ref_id/tgt_id/neg_id` row를 SigLIP token pair probe용 positive/negative row로 확장한다.
2. 기존 `tools/score_siglip_token_pair_probe.py` 결과를 C104 summary/report/per-row metrics로 변환하고,
   C098/Qwen 기준을 넘는지 결정한다.
...
PYTHONPATH=. /home/wktwin/anima-lora-training-bundle/anima_lora/.venv/bin/python \
  ...
  eval/c104_expanded_qwen_target_siglip_probe_20260613/probe_manifest.jsonl \
  eval/c104_expanded_qwen_target_siglip_probe_20260613/token_scores.json \
  --data-root .tmp/c097_siglip_hard_shape_expanded_root \
  --layer -6 \
  --topk 64 \
  --report-path eval/c104_expanded_qwen_target_siglip_probe_20260613/token_scores_report.md
```
...
- `eval/c104_expanded_qwen_target_siglip_probe_20260613/probe_manifest.jsonl`
- `eval/c104_expanded_qwen_target_siglip_probe_20260613/manifest_summary.json`
- `eval/c104_expanded_qwen_target_siglip_probe_20260613/token_scores.json`
- `eval/c104_expanded_qwen_target_siglip_probe_20260613/token_scores_report.md`
- `eval/c104_expanded_qwen_target_siglip_probe_20260613/per_row_metrics.jsonl`
- `eval/c104_expanded_qwen_target_siglip_probe_20260613/summary.json`
- `eval/c104_expanded_qwen_target_siglip_probe_20260613/report.md`

SigLIP2 layer `-6`, top-k `64` 기준 aggregate:

| metric | positive mean | negative mean | margin | AUC | decision |
|---|---:|---:|---:|---:|---|
| pooled | `0.8272018028` | `0.8165916013` | `0.0106102015` | `0.5057397959` | `token_feature_not_sufficiently_separated` |
| mean_max_token | `0.8044263508` | `0.7844510100` | `0.0199753408` | `0.7228954082` | `token_feature_not_sufficiently_separated` |
| topk_token | `0.9990838436` | `0.9990845940` | `-0.0000007504` | `0.4768813776` | `token_feature_not_sufficiently_separated` |

C104 summary의 최종 decision은 `c104_probe_not_enough_signal`이다.

best metric은 `mean_max_token`이지만, best margin은 `0.01997534079211105`이고 AUC는
`0.7228954081632653`이다. pass threshold는 Qwen baseline 기준 `0.1089544056`, AUC 기준 `0.85`로
잡았기 때문에 C104는 학습으로 진행할 신호가 부족하다고 판단했다.

### 판단

C104는 C097의 explicit negative 구조가 안전하게 확장되어 있고 경로 누락도 없다는 점은 확인했다.
하지만 SigLIP2 feature 공간 자체가 positive target과 explicit negative를 강하게 분리하지 못했다.
특히 top-k token은 거의 포화되어 positive/negative 구분에 실패했고, 가장 나은 mean-max token도
margin과 AUC 모두 C105 학습을 시작할 만큼 강하지 않다.
...
## 2026-06-14 C107 Qwen teacher distillation training/generation gate

### 왜 시작했나

C106에서 QwenVL teacher feature가 C097 hard-shape positive/explicit-negative 쌍을 매우 강하게
분리했다. teacher margin은 `0.21772855200937813`, AUC는 `1.0`이었다. 반대로 C104 SigLIP2 token
probe는 best margin `0.01997534079211105`, AUC `0.7228954081632653`에 머물렀다.

그래서 C107의 질문은 단순했다. “QwenVL teacher가 분리한 positive/negative 판단을 adapter 학습에
넣으면 실제 ComfyUI 생성 결과에서도 current best `blend_species_face`를 넘을 수 있는가?”였다.
...
- image root: `.tmp/c097_siglip_hard_shape_expanded_root`
- C107 manifest: `training/manifests/c107_qwen_teacher_distillation_20260613.jsonl`
- manifest summary: `training/manifests/c107_qwen_teacher_distillation_20260613.summary.json`
- rows written: `56`
- explicit negative rows: `56`
- heldout rows used: `0`
- teacher source: `C106`
- teacher margin mean: `0.21772855200937816`
- manifest decision: `c107_manifest_ready_for_bounded_training`

C107에서는 heldout을 학습에 넣지 않았다. C106 score에서 확인한 `neg_id` hard-negative 구조를 그대로
보존해 contrastive/retrieval 학습 row로 넘겼다.

### 개발한 것

C107 manifest/plan 생성 도구와 테스트를 추가했다.

- `tools/c107_qwen_teacher_distillation.py`
- `tests/test_c107_qwen_teacher_distillation.py`
- `docs/c107_qwen_teacher_distillation_plan_ko.md`

도구의 역할:

1. C106 QwenVL score JSON과 C097 manifest를 연결해 teacher-positive/explicit-negative row를 만든다.
2. positive/negative id mismatch를 감지해 잘못된 teacher row가 학습에 들어가지 않게 한다.
3. heldout row 사용량, 누락 경로, teacher margin min/mean/max, C106 source 정보를 summary에 기록한다.
...
이 checkpoint는 로컬/ignored 산출물이며 Git에는 올리지 않는다.

### ComfyUI generation gate

평가 폴더:

- `eval/c107_qwen_teacher_distillation_gate_20260613/`
...
| `blend_species_face` | previous retrieval `1.4` + C055 mixed `0.4` |
| `c107_qwen_teacher_w14` | C107 checkpoint `1.4` |
...
- generated PNG: `120`
- blank count: `0`
- contact sheets: `contact_sheet_train.jpg`, `contact_sheet_heldout.jpg`
- cleanup: port `8116` shutdown 후 `curl_exit=7`, `lsof` listener 없음

ComfyUI02 venv에는 `sentence_transformers`가 없어 첫 실행이 실패했다. root-owned venv를 직접 변경하지
않고 `.tmp/comfy_siglip_base/sentence_only` thin `PYTHONPATH`를 붙여 재시작했고, 이후 object_info와
생성 gate가 정상 완료됐다.
...
| variant | mean cosine | mean uplift | improved rate |
|---|---:|---:|---:|
| `blend_species_face` | `0.8150266468524933` | `0.060893215239048004` | `0.825` |
| `c107_qwen_teacher_w14` | `0.7785100042819977` | `0.0243765726685524` | `0.7` |

PE direct comparison에서 C107은 blend 대비 `11/40`만 승리했고, 평균 차이는 `-0.036516642570495604`였다.

QwenVL metric 전체:

| variant | mean cosine | mean uplift | improved rate |
|---|---:|---:|---:|
| `blend_species_face` | `0.8232675865292549` | `0.042190256714820865` | `0.8` |
| `c107_qwen_teacher_w14` | `0.8162184461951256` | `0.03514111638069153` | `0.75` |

QwenVL direct comparison에서 C107은 blend 대비 `14/40`만 승리했고, 전체 평균 차이는
`-0.007049140334129333`이었다. 단, heldout 평균만 보면 C107이 blend보다 `+0.00238955020904541`
높았다.

### 육안 감사

C107은 `no_ip` 대비 reference-control이 켜진다. 긴 흑발, 검붉은 의상, 보라색 기운, 붉은 눈,
상반신/측면 포즈 같은 큰 속성은 여러 샘플에서 reference 방향으로 이동했다.

하지만 current best `blend_species_face`보다 좋다고 보기 어렵다. C107은 대부분의 행에서 blend와
유사한 변형으로 보였고, 세부 identity lock이 더 강해지지 않았다. 특히 나이, 얼굴형, 수염, 대머리,
관모, 소품, 말풍선/크롭 맥락은 여전히 불안정했다.

### 최종 판단

결정: `c107_generation_gate_not_promoted`

C107은 “Qwen teacher score를 사용해 finite/loadable calibrator checkpoint를 만들 수 있다”는 증거다.
하지만 실제 생성 gate에서는 current best `blend_species_face`를 넘지 못했다. 따라서 standalone C107을
배포하거나 workflow default로 승격하지 않는다.

다음 루프는 teacher score를 단순 hard-negative manifest로만 쓰는 방식이 아니라, Qwen teacher
embedding/target 자체를 직접 정렬하거나 current best blend와 조합 가능한 route를 찾아야 한다.

### 산출물

- `docs/c107_qwen_teacher_distillation_plan_ko.md`
- `training/manifests/c107_qwen_teacher_distillation_20260613.jsonl`
- `training/manifests/c107_qwen_teacher_distillation_20260613.summary.json`
- `eval/c107_qwen_teacher_distillation_training_20260613/summary.json`
- `eval/c107_qwen_teacher_distillation_training_20260613/report.md`
- `eval/c107_qwen_teacher_distillation_training_20260613/train_stdout.txt`
- `eval/c107_qwen_teacher_distillation_gate_20260613/summary.json`
- `eval/c107_qwen_teacher_distillation_gate_20260613/metric_rollup.json`
- `eval/c107_qwen_teacher_distillation_gate_20260613/pe_similarity_metrics.json`
- `eval/c107_qwen_teacher_distillation_gate_20260613/qwenvl_similarity_metrics.json`
- `eval/c107_qwen_teacher_distillation_gate_20260613/contact_sheet_train.jpg`
- `eval/c107_qwen_teacher_distillation_gate_20260613/contact_sheet_heldout.jpg`
- `eval/c107_qwen_teacher_distillation_gate_20260613/visual_audit.md`
- `eval/c107_qwen_teacher_distillation_gate_20260613/visual_audit.json`
- `eval/c107_qwen_teacher_distillation_gate_20260613/report.md`

## 2026-06-15 - C108 Qwen teacher embedding target alignment

### 목적

C107은 QwenVL teacher score를 hard-negative 형태로 학습에 넣었지만, 실제 generation gate에서는
current best `blend_species_face`를 넘지 못했다. 실패 원인은 teacher signal이 positive/negative
score margin으로만 전달되고, target image 자체의 QwenVL embedding으로 adapter token을 직접 끌어당기지
못했다는 쪽으로 보았다.

C108의 목표는 reference image embedding을 adapter token으로 만든 뒤, pooled adapter token이 target
image의 QwenVL embedding에 가까워지도록 `teacher_alignment_loss`를 추가하는 것이다. runtime checkpoint
format은 그대로 유지하고, 새 loss는 학습 중에만 사용한다.
...
- heldout_rows_used: `0`
- missing_path_count: `0`
- teacher_target: `target_image_qwenvl_embedding`
...
| 항목 | weight |
|---|---:|
| base denoising | implicit |
| contrastive prediction margin | `0.35` |
| ref/wrong retrieval margin | `0.15` |
| target image QwenVL embedding alignment | `0.4` |

### 검증

먼저 2-step smoke로 실제 모델 로드, target image QwenVL embedding 준비, checkpoint 저장/검증을 확인했다.
이후 128-step bounded training을 실행했다.

학습 산출물:

- log: `eval/c108_qwen_teacher_embedding_alignment_training_20260615/train_stdout.txt`
- training summary: `eval/c108_qwen_teacher_embedding_alignment_training_20260615/training_summary.json`
- gate summary: `eval/c108_qwen_teacher_embedding_alignment_training_20260615/summary.json`
- report: `eval/c108_qwen_teacher_embedding_alignment_training_20260615/report.md`
- checkpoint: `checkpoints/anima_qwenvl_ip_adapter_c108_teacher_alignment_b128_0128_20260615.safetensors`
...
- `py_compile`: C108 학습/CLI/test 파일 통과
- focused pytest: `tests/test_qwenvl_teacher_alignment.py`,
  `tests/test_qwenvl_feature_calibration.py`, `tests/test_qwenvl_token_retrieval.py`에서 `14 passed`
- pure LOC: 새 Python 파일들은 모두 `250` pure LOC 이하
- read-only subagent audit: 구체적인 import/API/shape mismatch는 발견되지 않음

### 판단

결정: `proceed_to_c108_generation_gate`

C108 학습 gate는 통과했다. loss는 finite이고 checkpoint는 loadable이며 PE checkpoint rejection guard도 통과했다.
따라서 다음 단계는 isolated ComfyUI/API에서 `no_ip`, current best `blend_species_face`,
`c108_qwen_teacher_alignment_w14`를 같은 sample set으로 생성 비교하는 것이다.

주의점: `--train-calibrator-only`는 init checkpoint에 `feature_calibrator.*` weight가 이미 있어야 한다. 이번
init checkpoint인 `checkpoints/anima_qwenvl_ip_adapter_single_character_retrieval_0128_20260611.safetensors`는
조건을 만족해 smoke와 본 학습이 통과했다. plain QwenVL adapter를 init으로 쓰면 guard 실패가 정상이다.

### ComfyUI generation gate

평가 폴더:

- `eval/c108_qwen_teacher_embedding_alignment_gate_20260615/`
...
| `no_ip` | IP-Adapter 미적용 |
| `blend_species_face` | previous retrieval `1.4` + C055 mixed `0.4` |
| `c108_qwen_teacher_alignment_w14` | C108 checkpoint `1.4` |
...
- blank count: `0`
- contact sheets: `contact_sheet_train.jpg`, `contact_sheet_heldout.jpg`
- cleanup: port `8116` shutdown 후 `curl_exit=7`, listener 없음
...
|---|---:|---:|---:|
| `blend_species_face` | `0.8150266468524933` | `0.060893215239048004` | `0.825` |
| `c108_qwen_teacher_alignment_w14` | `0.7932347998023033` | `0.03910136818885803` | `0.7` |

PE direct comparison에서 C108은 blend 대비 `14/40`만 승리했고, 평균 차이는
`-0.021791847050189973`였다. heldout만 보면 C108의 PE uplift는 `-0.0041027069091796875`로
`no_ip`보다도 약했다.
...
|---|---:|---:|---:|
| `blend_species_face` | `0.8232675865292549` | `0.042190256714820865` | `0.8` |
| `c108_qwen_teacher_alignment_w14` | `0.8133075401186943` | `0.03223021030426025` | `0.75` |

QwenVL direct comparison에서 C108은 blend 대비 `13/40`만 승리했고, 평균 차이는
`-0.009960046410560608`였다.

육안 감사:

C108은 `no_ip` 대비 reference-control이 켜진다. 검붉은 팔레트, 보라색 기운, 긴 머리, 붉은 눈,
수염/관모 같은 큰 속성은 여러 샘플에서 reference 방향으로 이동했다. 하지만 대부분 `blend_species_face`와
유사하거나 더 약하다. heldout02 대머리/수염 노인은 blend가 더 가깝고, heldout05 bearded official/crop도
확실한 개선이 없다. 가장 중요한 heldout07 비인간 초록 측면 실루엣은 C108도 인간형 dark warrior로 붕괴해
해결하지 못했다.

최종 결정: `c108_generation_gate_not_promoted`

C108은 학습 objective가 runtime에 작동한다는 증거지만, current best를 넘는 생성 품질은 아니다. 따라서
standalone C108 checkpoint를 배포하거나 workflow default로 승격하지 않는다. 다음 루프는 target embedding
alignment를 더 오래 반복하는 것보다, current best blend와 조합 가능한 routing 또는 비인간 실루엣/말풍선/크롭
context를 직접 잡는 데이터/손실 방향을 우선한다.
