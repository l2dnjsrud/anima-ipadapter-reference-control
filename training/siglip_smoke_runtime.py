...
def encode_target_latents(
    ...
    pixels = load_anima_pixels(path, resolution=resolution).to(device=device, dtype=dtype)
    with torch.no_grad():
...
def encode_prompt(
    ...
    cond, _null = prepare_text_inputs(args, device, anima, shared_models={"text_encoder": text_encoder})
