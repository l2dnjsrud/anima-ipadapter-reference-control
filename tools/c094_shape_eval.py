...
C094_CHECKPOINT: Final = "anima_siglip_ip_adapter_c094_shape_supervised_0064_20260613.safetensors"
DEFAULT_C094_OUT_DIR: Final = Path(
    "eval/c094_siglip_shape_supervised_anti_collapse_generation_gate_20260613"
)


def c094_siglip_variants() -> tuple[Variant, ...]:
    return (
        ...
        Variant("c094_shape_supervised_w08", C094_CHECKPOINT, 0.8),
        Variant("c094_shape_supervised_w10", C094_CHECKPOINT, 1.0),
        Variant("c094_shape_supervised_w12", C094_CHECKPOINT, 1.2),
        Variant("c094_shape_supervised_w14", C094_CHECKPOINT, 1.4),
    )


def run_c094_eval(probe_manifest_path: Path, config: EvalConfig) -> JsonObject:
    ...
    rollup = write_c094_shape_metrics(config.out_dir / "summary.json", config.out_dir, audit)
    write_c094_report(config.out_dir, rollup, audit)
    return summary


def write_c094_shape_metrics(
    summary_path: Path,
    output_dir: Path,
    ...
    rollup = summarize_c094_shape_rows(
...
def write_c094_report(output_dir: Path, rollup: JsonObject, audit: JsonObject) -> None:
    lines = [
        "# c094 SigLIP Shape-Supervised Hard-Shape Gate",
        "",
        f"- Decision: `{rollup['decision']}`",
        f"- Best c094: `{rollup['best_c094_variant']}`",
        ...
        f"- C094 blank-like rows: `{json.dumps(rollup['c094_blank_like_rows'], ensure_ascii=False)}`",
...
def main(
    ...
    out_dir: Annotated[Path, typer.Option()] = DEFAULT_C094_OUT_DIR,
