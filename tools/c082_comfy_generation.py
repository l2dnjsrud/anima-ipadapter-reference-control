...
from tools.c082_single_image_pair_acquisition import C082Config, build_c082_prompt_package
...
class C082GenerationConfig:
    acquisition: C082Config = C082Config()
...
def run_c082_generation(config: C082GenerationConfig) -> JsonObject:
    build_c082_prompt_package(config.acquisition)
    ...
    summary = {
        "source": "c082_single_image_pair_generation",
        ...
        "blank_count": sum(1 for row in generated if row["blank"] is True),
...
def _run_prompt(row: JsonObject, config: C082GenerationConfig, generated_dir: Path) -> JsonObject:
...
def _workflow(row: JsonObject, config: C082GenerationConfig) -> JsonObject:
...
if __name__ == "__main__":
    run_c082_generation(C082GenerationConfig())
