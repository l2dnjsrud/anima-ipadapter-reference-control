...
def build_plan(inventory: JsonObject) -> str:
    ...
    return "\n".join(
        (
            ...
            "## C001 inventory 기준",
            "",
            f"- clean32 train rows: `{metrics['clean32_train_rows']}`",
            f"- clean32 heldout rows: `{metrics['clean32_heldout_rows']}`",
            f"- c052 positive pairs: `{metrics['c052_positive_pairs']}`",
            f"- c066 direct-green positive count: `{metrics['c066_direct_green_positive_count']}`",
            f"- c097 selected synthetic hard-shape rows: `{metrics['c097_selected_rows']}`",
            "",
...
def build_report(summary: JsonObject, inventory: JsonObject) -> str:
    return "\n".join(
        (
            ...
            f"- omitted_heldout_rows: `{summary['omitted_heldout_rows']}`",
            f"- real_local_rows: `{summary['real_local_rows']}`",
            f"- real_local_direct_green_confirmed_rows: `{summary['real_local_direct_green_confirmed_rows']}`",
            f"- external_direct_green_positive_rows: `{summary['external_direct_green_positive_rows']}`",
            f"- synthetic_hard_shape_rows: `{summary['synthetic_hard_shape_rows']}`",
            "",
            "## 판단",
            "",
            str(summary["blocker_reason"] or summary["next_c100_command_surface"]),
            "",
            "## 인벤토리 핵심",
            "",
