...
C091_CHECKPOINT: Final = "anima_siglip_ip_adapter_c091_feature_calibrator_b64_0064_20260613.safetensors"
DEFAULT_C091_OUT_DIR: Final = Path("eval/c091_siglip_feature_calibrator_hard_shape_gate_20260613")


def c091_siglip_variants() -> tuple[Variant, ...]:
    return (
        Variant("no_ip", None, 0.0),
        Variant("siglip_pilot_w14", SIGLIP_PILOT_CHECKPOINT, 1.4),
        Variant("c089_shape_w14", C089_CHECKPOINT, 1.4),
        Variant("c091_feature_calibrator_w10", C091_CHECKPOINT, 1.0),
        Variant("c091_feature_calibrator_w14", C091_CHECKPOINT, 1.4),
    )


def run_c091_eval(probe_manifest_path: Path, config: EvalConfig) -> JsonObject:
    ...
    rollup = write_c091_shape_metrics(config.out_dir / "summary.json", config.out_dir)
    audit = write_pixel_nonblank_audit(results, config.out_dir / PIXEL_AUDIT_NAME)
    write_c091_report(config.out_dir, rollup, audit)
    return summary


def write_c091_shape_metrics(summary_path: Path, output_dir: Path) -> JsonObject:
    ...
    rollup = summarize_c091_shape_rows(tuple(row for row in rows if isinstance(row, dict)))
...
def summarize_c091_shape_rows(rows: tuple[JsonObject, ...]) -> JsonObject:
    summaries = _variant_summaries(rows)
    c091 = _best_prefixed(summaries, "c091_")
    ...
    return {
        ...
        "decision": _c091_decision(c091, c089, qwen),
    }


def write_c091_report(output_dir: Path, rollup: JsonObject, audit: JsonObject) -> None:
    lines = [
        "# c091 SigLIP Feature-Calibrator Hard-Shape Gate",
        "",
        f"- Decision: `{rollup['decision']}`",
        f"- Best c091: `{rollup['best_c091_variant']}`",
...
def _c091_decision(c091: tuple[str, float], c089: tuple[str, float], qwen: tuple[str, float]) -> str:
    if c091[1] >= qwen[1] - 0.01:
        return "c091_feature_calibrator_candidate_for_larger_gate"
    if c091[1] >= c089[1] + 0.01:
        return "c091_improves_c089_but_not_qwen_baseline"
    if c091[1] >= c089[1] - 0.005:
        return "c091_matches_c089_not_qwen_baseline"
    return "c091_not_improved_requires_true_encoder_checkpoint"
...
def main(
    ...
    out_dir: Annotated[Path, typer.Option()] = DEFAULT_C091_OUT_DIR,
