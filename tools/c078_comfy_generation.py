...
from tools.c078_synthetic_bootstrap import C078BootstrapConfig, build_c078_prompt_package
...
class C078GenerationConfig:
    bootstrap: C078BootstrapConfig = C078BootstrapConfig()
...
def run_c078_generation(config: C078GenerationConfig) -> JsonObject:
    build_c078_prompt_package(config.bootstrap)
    rows = _read_jsonl(config.bootstrap.out_dir / "prompt_manifest.jsonl")
    generated_dir = config.bootstrap.scratch_dir / "generated"
    generated_dir.mkdir(parents=True, exist_ok=True)
    generation_rows: list[JsonObject] = []
    for row in rows:
        result = _run_prompt(row, config, generated_dir)
        generation_rows.append(result)
    manifest_path = config.bootstrap.out_dir / "generation_manifest.jsonl"
    manifest_path.write_text("".join(json.dumps(row, ensure_ascii=False) + "\n" for row in generation_rows), encoding="utf-8")
    summary = {
        "source": "c078_synthetic_direct_green_bootstrap_generation",
        "base_url": config.base_url,
        "prompt_count": len(rows),
        "generated_count": sum(1 for row in generation_rows if row["status"] == "generated"),
        "failed_count": sum(1 for row in generation_rows if row["status"] != "generated"),
        "generation_manifest": str(manifest_path),
        "heldout_rows_used": 0,
        "training_started": False,
    }
    (config.bootstrap.out_dir / "generation_summary.json").write_text(json.dumps(summary, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    return summary


def _run_prompt(row: JsonObject, config: C078GenerationConfig, generated_dir: Path) -> JsonObject:
    candidate_id = str(row["candidate_id"])
    workflow = _workflow(row, config)
    response = post_json(config.base_url, "/prompt", {"prompt": workflow})
...
def _workflow(row: JsonObject, config: C078GenerationConfig) -> JsonObject:
    prompt = str(row["prompt"])
    negative = str(row["negative"])
    prefix = f"anima_ipadapter/{config.bootstrap.out_dir.name}/{row['candidate_id']}"
    return {
...
def _text_node(text: str) -> JsonObject:
    return {"class_type": "CLIPTextEncodeFlux", "inputs": {"clip": ["6", 0], "clip_l": text, "t5xxl": text, "guidance": 3.5}}
...
if __name__ == "__main__":
    run_c078_generation(C078GenerationConfig())
